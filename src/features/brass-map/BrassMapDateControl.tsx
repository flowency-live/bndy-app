...
export function BrassMapDateControl({ sel, onChange, concerts, today }: {
  sel: MapDateSel;
  onChange: (selection: MapDateSel) => void;
  concerts: Gig[];
  today: string;
}) {
  ...
  const countOn = (predicate: (date: string) => boolean) => concerts.reduce((total, concert) => predicate(concert.date) ? total + 1 : total, 0);
  const dayCounts = new Map<string, number>();
  for (const concert of concerts) dayCounts.set(concert.date, (dayCounts.get(concert.date) ?? 0) + 1);

  const start = addDaysISO(today, -mondayIdx(today));
  const last = addDaysISO(endWin, 6 - mondayIdx(endWin));
  const cells: string[] = [];
  for (let date = start; date <= last; date = addDaysISO(date, 1)) cells.push(date);
  const monthA = MON[Number(today.slice(5, 7)) - 1];
  const monthB = MON[Number(endWin.slice(5, 7)) - 1];
  const monthLabel = monthA === monthB ? monthA : `${monthA} – ${monthB}`;

  const pick = (selection: MapDateSel) => { onChange(selection); setOpen(false); };
  const step = (delta: number) => {
    if (!focused) return;
    const date = addDaysISO(focused, delta);
    if (date >= today && date <= endWin) onChange(selFromDay(date, today));
  };
    ...
    <>
      ...
      <Sheet open={open} onClose={() => setOpen(false)}>
        ...
        <div className="mb-4 grid grid-cols-3 gap-2">
          <Quick label="Today" count={countOn((date) => date === today)} on={sel.kind === "today"} onClick={() => pick({ kind: "today" })} />
          <Quick label="This weekend" count={countOn((date) => weekendDates("this", today).includes(date))} on={sel.kind === "weekend" && sel.which === "this"} onClick={() => pick({ kind: "weekend", which: "this" })} />
          <Quick label="Next weekend" count={countOn((date) => weekendDates("next", today).includes(date))} on={sel.kind === "weekend" && sel.which === "next"} onClick={() => pick({ kind: "weekend", which: "next" })} />
        </div>
        ...
        <div className="grid grid-cols-7 gap-1" role="group" aria-label="Choose a date">
          {cells.map((date) => {
            const inRange = date >= today && date <= endWin;
            const selected = sel.kind !== "weekend" && matchesMapDate(date, sel, today);
            const isToday = date === today;
            const count = dayCounts.get(date) ?? 0;
            return (
              <button key={date} type="button" disabled={!inRange}
                aria-label={`${fullDateLabel(date)}, ${count} concert${count === 1 ? "" : "s"}`}
                aria-pressed={inRange ? selected : undefined}
                onClick={() => pick(selFromDay(date, today))}
                ...
                {inRange && count > 0 && !selected && <span aria-hidden="true" className="absolute bottom-1 h-1 w-1 rounded-full bg-acc" />}
              </button>
            );
          })}
        </div>

        <Link href="/concerts" className="mt-5 flex items-center justify-center gap-1.5 rounded-2xl border border-line bg-white/5 py-3 text-[13px] font-bold text-dim">
          Planning further ahead? Browse all concerts <ArrowRight size={15} />
        </Link>
      </Sheet>
    </>
  );
...
function Quick({ label, count, on, onClick }: { label: string; count: number; on: boolean; onClick: () => void }) {
  return (
    <button type="button" aria-pressed={on} onClick={onClick}
      className={cn("flex flex-col items-center gap-0.5 rounded-2xl border py-3 text-center transition-colors", on ? "bndy-chip on" : "bndy-chip text-dim hover:text-txt")}>
      <span className="text-[12.5px] font-extrabold leading-tight">{label}</span>
      <span className={cn("text-[10px] font-bold", on ? "" : "text-dim2")}>{count} concert{count === 1 ? "" : "s"}</span>
    </button>
  );
}
