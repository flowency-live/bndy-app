  ...
  return (
    ...
    >
      ...
      <div className="relative hidden h-full items-center justify-center lg:flex" aria-hidden>
        ...
        <span className={cn("relative z-[1] h-2.5 w-2.5 rounded-full border-2 border-[var(--surface)] bg-dim2 shadow-[0_0_0_1px_color-mix(in_srgb,var(--line)_65%,transparent)] transition-transform duration-150 group-hover:scale-125", tonight && !gig.cancelled && "bg-acc shadow-[0_0_0_1px_var(--acc)]")} />
      </div>

      <div className="transition-transform duration-200 ease-smooth group-hover:scale-[1.045]">
        {gig.isOpenMic && !imageUrl ? (
          <><div className="lg:hidden"><MicTile size={56} radius={13} /></div><div className="hidden lg:block"><MicTile size={72} radius={16} /></div></>
        ) : (
          <><div className="lg:hidden"><Avatar id={lead?.id || gig.venueId} name={lead?.name || gig.venueName} src={imageUrl} size={56} radius={13} /></div><div className="hidden lg:block"><Avatar id={lead?.id || gig.venueId} name={lead?.name || gig.venueName} src={imageUrl} size={72} radius={16} /></div></>
        )}
      </div>
      ...
      <div className="hidden text-right lg:block">{hasDistance && <span className="tnum text-[12px] font-extrabold text-dim">{formatDistance(distance)}</span>}</div>
      <ChevronRight size={18} className="shrink-0 text-dim2 transition-transform duration-150 group-hover:translate-x-1" />
    </button>
  );
});

const TONE: Record<string, string> = {
  cancelled: "bg-red-500/15 text-red-700 dark:text-red-300 uppercase tracking-wide",
