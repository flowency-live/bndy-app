...
export function LocationField({ value, onChange }: { value: OriginChoice; onChange: (o: OriginChoice) => void }) {
  ...
  useEffect(() => {
    ...
    if (q.trim().length < 2) { setPreds([]); return; }
    deb.current = window.setTimeout(async () => {
      setBusy(true);
      try { setPreds(await placesSuggest(q, "town")); } catch { setPreds([]); }
      setBusy(false);
    }, 220);
          ...
          {(busy || preds.length > 0 || q.trim().length >= 2) && (
            <div className="mt-1 max-h-56 overflow-y-auto">
              ...
              {preds.slice(0, 6).map((p) => (
                <button key={p.placeId} onClick={() => pickPlace(p)} className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left transition hover:bg-white/5">
