    ...
    try {
      ...
      const matches: ArtistCandidate[] = result.candidates.length
        ...
        : result.action === "matched" && result.artistId
          ? [{
              id: result.artistId,
              name: result.artistName ?? name.trim(),
              location: result.artistLocation ?? loc,
      ...
      <section className="mt-7 space-y-4 rounded-[26px] border border-line glass p-5 sm:p-6">
        ...
        {name.trim().length >= 2 && autocompleteDone && candidates.length > 0 && (
          <div className="overflow-hidden rounded-2xl border border-line">
            <div className="border-b border-line px-4 py-2.5 text-[10px] font-black uppercase tracking-[1.1px] text-dim">Matching artists</div>
            {candidates.map((candidate) => (
              <button key={candidate.id} type="button" onClick={() => startClaim(candidate)} className="flex w-full items-center justify-between gap-4 border-b border-line px-4 py-3 text-left last:border-0 hover:bg-white/5">
                <span className="min-w-0"><span className="block truncate text-[14px] font-black text-txt">{candidate.name}</span><span className="mt-0.5 flex items-center gap-1 text-[11px] font-bold text-dim"><MapPin size={11} /> {candidate.location || "Location not listed"}</span></span>
                <span className="shrink-0 text-[11px] font-black text-[var(--acc-text)]">That&apos;s us <ArrowRight size={13} className="ml-1 inline" /></span>
              </button>
            ))}
          </div>
        )}
        ...
        {creatingNew && (
          <div className="space-y-4 border-t border-line pt-4">
            ...
            {locationMatches.length > 0 && <div className="rounded-2xl border border-[var(--acc)]/45 p-3"><div className="text-[11.5px] font-black">That name and location may already exist.</div><div className="mt-2 space-y-2">{locationMatches.map((candidate) => <button key={candidate.id} type="button" onClick={() => startClaim(candidate)} className="flex w-full items-center justify-between rounded-xl border border-line px-3 py-2.5 text-left hover:bg-white/5"><span><span className="block text-[13px] font-black">{candidate.name}</span><span className="mt-0.5 flex items-center gap-1 text-[10.5px] font-bold text-dim"><MapPin size={10} /> {candidate.location || "Location not listed"}</span></span><span className="text-[10.5px] font-black text-[var(--acc-text)]">Choose</span></button>)}</div></div>}
