...
export function JoinArtistFlow() {
  ...
  useEffect(() => {
    ...
    setConfirmedDistinct(saved.confirmNew === true);
  ...
  const continueToNewArtist = (loc: string, confirmNew: boolean) => {
    setConfirmedDistinct(confirmNew);
    saveJoinState({ entityType: "artist", intent: "new", name: name.trim(), location: loc, confirmNew: confirmNew || undefined });
  ...
  const confirmDistinctArtist = async () => {
    if (!name.trim() || !location.trim() || locationMatches.length === 0) return;
    const loc = pickedLocation ?? location.trim();
    setLoading(true);
    setError(null);
    try {
      const result = await resolveArtist({ name: name.trim(), location: loc }, { dryRun: true, confirmNew: true });
      if (result.action !== "clear") {
        setError(result.message || "We couldn't safely confirm this as a different artist. Nothing has been created.");
        return;
      }
      continueToNewArtist(loc, true);
    } catch {
      setError("We couldn't safely confirm this as a different artist. Nothing has been created.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createOwnedArtist = async () => {
    ...
    try {
      const result = await joinArtist({
        ...
        confirmNew: confirmedDistinct || undefined,
      });
      if (result.ok) {
        clearJoinState();
    ...
    return (
      <main className="mx-auto max-w-xl px-4 pb-36 pt-10 lg:pt-14">
        <section className="text-center">
          ...
          <div className="mt-5 font-meta text-[9px] font-black uppercase tracking-[1.6px] text-[var(--acc-text)]">Artist added</div>
  ...
  if (phase === "new") {
    return (
      <main className="mx-auto max-w-xl px-4 pb-36 pt-6 lg:pt-10">
        ...
        <AuthGate title="Sign in to add this artist">
          <section className="mt-7 space-y-5 rounded-[24px] border border-[var(--acc)] glass p-5">
            ...
            <button type="button" disabled={loading || !artistType} onClick={createOwnedArtist} className="bndy-btn2 flex min-h-12 w-full items-center justify-center gap-2 px-4 text-[12px] disabled:opacity-50">{loading ? <Loader2 size={15} className="animate-spin" /> : <BadgeCheck size={15} />} Add this artist</button>
          </section>
        </AuthGate>
      </main>
    );
      ...
      <section className="mt-7 space-y-4 rounded-[26px] border border-line glass p-5 sm:p-6">
        ...
        {name.trim().length >= 2 && autocompleteDone && !creatingNew && (
          <button type="button" onClick={beginCreateNew} className="bndy-btn flex min-h-11 w-full items-center justify-center gap-2 px-4 text-[12px]">
            {candidates.length > 0 ? `Add a new ${name.trim()} in another location` : `Add ${name.trim()} as a new artist`}
            <ArrowRight size={14} />
          </button>
        )}
        ...
        {creatingNew && (
          <div className="space-y-4 border-t border-line pt-4">
            ...
            {locationMatches.length > 0 && <div className="rounded-2xl border border-line p-3"><div className="text-[11.5px] font-black">Is this the existing artist?</div><p className="mt-1 text-[10.5px] font-semibold text-dim">Choose the matching page, or confirm that yours is a different artist in {pickedLocation ?? location.trim()}.</p><div className="mt-1 divide-y divide-line">{locationMatches.map((candidate) => <ArtistCandidateRow key={candidate.id} candidate={candidate} onChoose={() => startClaim(candidate)} />)}</div><button type="button" disabled={loading} onClick={confirmDistinctArtist} className="bndy-btn2 mt-3 flex min-h-12 w-full items-center justify-center gap-2 px-4 text-[12px] disabled:opacity-40">{loading ? <Loader2 size={15} className="animate-spin" /> : <ArrowRight size={15} />} Different artist in {pickedLocation ?? location.trim()}</button></div>}

            {locationMatches.length === 0 && <button type="button" disabled={!location.trim() || loading} onClick={confirmNewLocation} className="bndy-btn2 flex min-h-12 w-full items-center justify-center gap-2 px-5 text-[13px] disabled:opacity-40">{loading ? <Loader2 size={16} className="animate-spin" /> : <Search size={16} />} Continue with this location</button>}
          </div>
        )}
      </section>
