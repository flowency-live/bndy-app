...
export function JoinVenueFlow() {
  ...
  useEffect(() => {
    ...
    if (phase !== "search" || query.trim().length < 2 || selected?.name === query) { setPlaces([]); return; }
    let cancelled = false;
  ...
  const choose = async (place: PlaceSuggestion) => {
    setQuery(place.name); setPlaces([]); setLoading(true); setError(null);
    const details = await placesDetails(place.placeId).catch(() => null);
    setLoading(false);
    if (!details) { setError("We couldn't load that place. Pick it again or try another result."); return; }
    setSelected(details);
  };
  ...
  if (phase === "success" && joined) return (
    <main className="mx-auto max-w-xl px-4 pb-36 pt-10 lg:pt-14"><section className="text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-acc text-on-acc"><CheckCircle2 size={30} /></span><div className="mt-5 font-meta text-[9px] font-black uppercase tracking-[1.6px] text-[var(--acc-text)]">You&apos;re on bndy</div><h1 className="font-disp mt-1 text-[38px] font-black leading-none tracking-tight">{joined.name}</h1>{joined.address && <p className="mx-auto mt-2 flex max-w-md items-start justify-center gap-1.5 text-[12px] font-bold text-dim"><MapPin size={13} className="mt-0.5" /> {joined.address}</p>}<p className="mx-auto mt-4 max-w-md text-[13px] font-semibold leading-relaxed text-dim">The venue exists and your account is its owner. From here you can manage its bndy presence without sharing passwords.</p><div className="mx-auto mt-7 grid max-w-md gap-2 sm:grid-cols-2"><Link href={`/venues/${joined.id}`} className="bndy-btn2 flex min-h-11 items-center justify-center px-4 text-[12px]">View venue</Link><Link href="/join" className="bndy-btn flex min-h-11 items-center justify-center px-4 text-[12px]">Done</Link></div></section></main>
  );
