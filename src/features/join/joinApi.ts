...
export interface JoinVenueCandidate {
  id: string;
  name: string;
  address?: string;
  city?: string;
...
export async function checkJoinVenue(input: JoinVenueIdentity): Promise<{ existing: JoinVenueCandidate | null; clear: boolean; message?: string }> {
  const response = await fetch(`${BASE}/api/community/venues/find-or-create`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...input, canCreate: false, source: "join_bndy_preflight" }),
  });
  const body = await response.json().catch(() => ({})) as Record<string, unknown>;
...
export async function joinVenue(input: JoinVenueIdentity): Promise<JoinVenueResult> {
  const response = await fetch(`${BASE}/api/join/venues`, {
    method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input),
  });
  const body = await response.json().catch(() => ({})) as Record<string, unknown>;
