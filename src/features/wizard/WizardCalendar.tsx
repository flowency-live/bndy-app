...
/** Inline single-date calendar — same visual language as GigDatePicker, no sheet,
 *  one tap to pick. `dots` marks dates that already have gigs (busy-night hint). */
  ...
  return (
    <div className="rounded-2xl border border-line glass p-3">
      <div className="mb-1.5 flex items-center justify-between">
        <button type="button" aria-label="Previous month" disabled={!canPrev} onClick={() => shiftMonth(-1)}
        ...
        <button type="button" aria-label="Next month" disabled={!canNext} onClick={() => shiftMonth(1)}
        ...
        {cells.map((d) => {
          ...
          return (
            <button key={d} type="button" onClick={() => onPick(d)}
              className={cn("relative flex h-10 items-center justify-center rounded-lg text-[13.5px] font-bold transition-colors",
                !inMonth && "text-dim2/50",
                inMonth && !isSel && "text-txt hover:bg-white/5",
                isSel && "bg-acc font-black text-on-acc",
                isToday && !isSel && "ring-1 ring-inset ring-[var(--acc)]")}>
              {Number(d.slice(8, 10))}
              {busy && !isSel && <span className="absolute bottom-1 h-1 w-1 rounded-full bg-[var(--acc2)]" />}
            </button>
          );
        })}
      </div>
