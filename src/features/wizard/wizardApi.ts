...
async function call<T>(method: "GET" | "POST", path: string, body?: unknown): Promise<{ status: number; body: T }> {
  ...
  let parsed: T; try { parsed = (await res.json()) as T; } catch { parsed = {} as T; }
...
async function recordCuratorCreation(entityType: "artist" | "venue" | "event", entityId: string | undefined): Promise<void> {
  if (!entityId) return;
  try {
    const auth = await checkAuth(); if (auth?.user.role !== "curator") return;
    const res = await fetch(`${BASE}/users/profile`, { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ curatorCreated: { entityType, entityId } }) });
...
export async function placesSuggest(q: string, kind: "venue" | "town" = "venue"): Promise<PlaceSuggestion[]> { const { status, body } = await call<{ suggestions?: PlaceSuggestion[] }>("GET", `/api/places/suggest?q=${encodeURIComponent(q)}${kind === "town" ? "&kind=town" : ""}`); return status === 200 ? body.suggestions ?? [] : []; }
...
export async function placesDetails(placeId: string): Promise<PlaceDetails | null> { const { status, body } = await call<{ place?: PlaceDetails }>("GET", `/api/places/details?placeId=${encodeURIComponent(placeId)}`); return status === 200 ? body.place ?? (body as unknown as PlaceDetails) ?? null : null; }
...
export async function findOrCreateVenue(input: { name: string; address?: string; city?: string; googlePlaceId?: string; latitude?: number; longitude?: number; socialMediaUrls?: string[] }): Promise<VenueResult> {
  const { status, body } = await call<Record<string, unknown>>("POST", "/api/community/venues/find-or-create", { ...input, source: "community_wizard" }); const v = (body.venue ?? body) as Record<string, unknown>; const id = (v.id ?? body.venueId ?? body.existingId) as string | undefined;
...
export async function resolveArtist(input: { name: string; location: string; facebookUrl?: string; profileImageUrl?: string; verifiedSourceName?: boolean; bio?: string; websiteUrl?: string; genres?: string[]; artistType?: string; actType?: string[]; acoustic?: boolean }, opts?: { dryRun?: boolean; confirmNew?: boolean; resolveTo?: string }): Promise<ArtistResolution> {
  ...
  const artist = (body.artist ?? {}) as Record<string, unknown>; const artistId = (body.artistId ?? artist.id ?? body.existingArtistId ?? body.existingId) as string | undefined; const rawCandidates = (body.candidates ?? body.matches ?? []) as Array<Record<string, unknown>>;
  ...
  const action = (body.action as ArtistResolution["action"]) ?? undefined; const matchedBy = body.matchedBy as string | undefined; const matchedVariant = (body.variant ?? body.matchedVariant) as string | undefined;
  ...
  if (action) { if (action === "created" && artistId) await recordCuratorCreation("artist", artistId); return { action, artistId, artistName: (artist.name as string) ?? input.name, artistLocation: (artist.location as string) ?? input.location, candidates, matchedBy, matchedVariant, code: body.code as string, message: (body.message ?? body.error) as string }; }
...
export async function createCommunityEvent(payload: { artistId?: string; artistIds?: string[]; headlineArtistIds?: string[]; isOpenMic?: boolean; venueId: string; date: string; startTime: string; endTime?: string; title?: string; ticketed?: boolean; ticketUrl?: string; ticketInformation?: string; imageUrl?: string; description?: string; festivalId?: string; festivalName?: string; hp?: string; startedAt?: number }): Promise<EventResult> {
  const { status, body } = await call<Record<string, unknown>>("POST", "/api/community/events", { ...payload, source: "community_wizard" }); const eventId = (body.eventId ?? (body.event as Record<string, unknown> | undefined)?.id ?? body.id) as string | undefined;
  if (status === 200 || status === 201) { if (status === 201) await recordCuratorCreation("event", eventId); return { ok: true, eventId }; }
